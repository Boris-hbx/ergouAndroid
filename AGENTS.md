# 二狗A（Android 端 Agent）工程规范

> **你是二狗A**（Android 端 Agent，Kotlin + Jetpack Compose）。看到 `@二狗A` 就是叫你接令。
> **接令规则**：看到任务后先 emit `task.started`（状态变 🟡）；完成后 emit `task.submitted` 并通知 PM 复验——**结令是 PM 的事（见铁律 5）**。
> 完整权威铁律集以 `C:\Project\ergouPM\CLAUDE.md` 为准；本文件是 Android 端执行者视角的同步副本。
...
| 文件 | 绝对路径 | 用途 |
|------|---------|------|
| 任务看板 | `C:\Project\ergouPM\docs\task-board.md` | 任务令正文（T-xxx 统一编号），找 `@二狗A` |
| 事件账本 | `C:\Project\ergouPM\events\ledger.jsonl` | 事件溯源，状态变更唯一真相源 |
| 事件工具 | `C:\Project\ergouPM\scripts\emit-event.js` | 记录事件（接令/试错/提交/完成） |
| 查我的令 | `C:\Project\ergouPM\scripts\list-open-tasks.js` | `--agent A --waiting` 查待接令（铁律 4） |
...
- 用户反馈修改有问题时，先记一次失败：
  `node C:/Project/ergouPM/scripts/emit-event.js task.attempt --task {T-xxx} --by AgentA --result FAIL --reason "用户反馈: {问题}"`
...
2. **emit started**：`node C:/Project/ergouPM/scripts/emit-event.js task.started --task {T-xxx} --by AgentA`，确认状态变 🟡。
3. **再动手**：状态确认 🟡 后才写代码。

**完工时**：先写复盘 `C:\Project\ergouPM\retro\{T-xxx}.A-codex.md`（ADR-010 硬闸门，缺则结令被拒）→ submit 交回 PM 复验。
...
```
node C:/Project/ergouPM/scripts/list-open-tasks.js --agent A --waiting
```
...
**你（二狗A，执行端）的职责终点是「push + 开/更新 PR + emit `task.submitted` + 通知 PM 复验」。以下动作属于 PM / 发版职责，你不得自行实施，即便 Boris 在终端口头授权也不行——必须回到 PM（Claude）走令：**
...
```bash
# 接令
node C:/Project/ergouPM/scripts/emit-event.js task.started --task {T-xxx} --by AgentA
# 提交（交回 PM 复验；结令是 PM 的事）
node C:/Project/ergouPM/scripts/emit-event.js task.submitted --task {T-xxx} --by AgentA --summary "简述"
```
...
- 执行者留痕：接令/提交统一 `--by A-codex` / `A-claude`。
...
## 端信息
- 技术栈：Kotlin + Jetpack Compose；仓库 `Boris-hbx/ergouAndroid`。
