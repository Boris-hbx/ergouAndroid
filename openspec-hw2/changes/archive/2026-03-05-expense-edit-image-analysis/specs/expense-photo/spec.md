...
The system SHALL provide two ways to select a photo: pick from gallery and capture with camera. The system SHALL use Android ActivityResult APIs (`PickVisualMedia` for gallery, `TakePicture` for camera). Camera capture SHALL request `CAMERA` permission at runtime.

#### Scenario: Pick photo from gallery
- **WHEN** user selects "from gallery" option
- **THEN** system launches the photo picker and returns the selected image URI
...
The system SHALL support sending a photo to `POST /api/expenses/parse-preview` for AI analysis. The request SHALL send the image as base64 in the request body. The system SHALL display a preview of the parsed result and allow the user to edit before saving.

#### Scenario: Successful AI parse
- **WHEN** user initiates "scan receipt" and selects a photo
- **THEN** system sends the photo to parse-preview API with a loading indicator
- **THEN** system displays the parsed result: merchant (as notes), date, total amount, currency, tags, and item lines

#### Scenario: AI parse timeout
- **WHEN** the parse-preview API does not respond within 120 seconds
- **THEN** system displays a timeout message and offers manual input as fallback

#### Scenario: AI parse returns empty or invalid result
- **WHEN** the API returns an empty items list or missing amount
- **THEN** system displays a message that parsing failed and offers manual input
...
- **THEN** item lines are displayed read-only (editable in future iteration)

### Requirement: Save parsed receipt as new expense
The system SHALL create a new expense entry from the confirmed parse preview. The system SHALL call `POST /api/expenses` with the confirmed fields and items array, then upload the original photo to the new entry.

#### Scenario: Save parsed receipt
- **WHEN** user confirms the parse preview
- **THEN** system creates the expense via API with items array
- **THEN** system uploads the original photo to the new expense entry
- **THEN** system navigates to the expense list with refreshed data

#### Scenario: Save fails
- **WHEN** the create API call fails
- **THEN** system displays an error and preserves the preview data for retry

#### Scenario: Photo upload fails after successful create
- **WHEN** the expense is created but photo upload fails
- **THEN** system still navigates to the list (expense saved without photo)
