...
1. **先接令**：`node C:\Project\ergouPM\scripts\emit-event.js task.started --task T-xxx --by AgentA`（令须派给 A、未接过）
2. 建分支：`git checkout -b feat/T-xxx-简述`
3. 提交：commit message **必须含 `T-xxx`**（如 `feat: T-123 ...`）
4. push 分支 → 开 PR 到 main
5. **task-gate** 校验：该令在 ergouPM ledger 存在、派给 A、已接令、ledger 未篡改 → 绿才能合并
6. 无令/越权/未接令 → PR 红 → 合不进

闸门读 GitHub 上的 ergouPM ledger（emit-event 自动同步）。**务必先接令再提交**，否则闸门查无此令、PR 被拒。
紧急绕过（仅 Boris）：`gh api --method DELETE repos/Boris-hbx/ergouAndroid/branches/main/protection`
